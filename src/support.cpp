...
void AreaSupport::generateSupportAreasForMesh(SliceDataStorage& storage, const Settings& infill_settings, const Settings& roof_settings, const Settings& bottom_settings, const size_t mesh_idx, const size_t layer_count, std::vector<Polygons>& support_areas)
{
    ...
    #pragma omp parallel for default(none) shared(xy_disallowed_per_layer, storage, mesh) firstprivate(layer_count,is_support_mesh_place_holder,use_xy_distance_overhang,z_distance_top,tan_angle,xy_distance,xy_distance_overhang,layer_z_distance_top) schedule(dynamic)
    // Use a signed type for the loop counter so MSVC compiles (because it uses OpenMP 2.0, an old version).
...
#pragma omp parallel for default(none) shared(support_areas, storage) firstprivate(max_checking_layer_idx,layer_z_distance_top) schedule(dynamic)
        // Use a signed type for the loop counter so MSVC compiles (because it uses OpenMP 2.0, an old version).
